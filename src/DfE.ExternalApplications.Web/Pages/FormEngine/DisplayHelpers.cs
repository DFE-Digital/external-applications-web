...
public static class DisplayHelpers
{
    ...
    private static string InterpolateJsonValue(string message, string key, JsonElement jsonElement)
    {
        ...
        {
            if (!JsonPath.TryParse($"$.{subkey}", out var path)) continue;
            
            var node = JsonSerializer.SerializeToNode(jsonElement);
            var result = path.Evaluate(node);

            if (result.Matches.Count <= 0) continue;
            
            var placeholder = $"{{{key}.{subkey}}}";
            message = message.Replace(placeholder, result.Matches[0].Value?.ToString()) ?? "";
        }
